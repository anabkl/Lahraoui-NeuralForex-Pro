...
def get_ohlcv() -> pd.DataFrame:
    """Return the best available OHLCV DataFrame (hourly preferred, 1-min fallback)."""
    df = download_hourly(730)
    if df.empty:
        logger.warning("Hourly data unavailable – falling back to 1-minute data")
        df = download_minutely(7)
    if df.empty:
        raise RuntimeError(f"Could not download any data for {TICKER}")
